...
try:
    ...
    _HADAMARD_KERNEL_AVAILABLE = True
except ImportError:
    _HADAMARD_KERNEL_AVAILABLE = False
import sys
import numpy as np

try:
    from .ffht_backend import fwht_axis0_inplace as _ffht_axis0_inplace
except ImportError:
    _ffht_axis0_inplace = None
...
def fastwht(x, N=0, order='sequency'):
    ...
    if not _HADAMARD_KERNEL_AVAILABLE:
        if order != 'hadamard':
            raise ImportError("fastwht: hadamardKernel is unavailable, and only hadamard-ordered FFHT fallback is supported")
        y = orthonormal_hadamard(y) / np.sqrt(float(N))
...
def orthonormal_hadamard(x):
    """
    Apply the orthonormal Walsh-Hadamard transform along axis 0.

    When the optional FFHT extension is available, use it for float32/float64
    inputs whose first dimension is already a power of two. Otherwise, fall
    back to the legacy SWIG-backed implementation.
    """
    arr = np.asarray(x)
    if arr.ndim < 1 or arr.ndim > 2:
        raise TypeError("orthonormal_hadamard: x must be one or two dimensional")

    n = arr.shape[0] if arr.ndim > 1 else max(arr.shape)
    if not _is_power_of_2(n):
        raise ValueError("orthonormal_hadamard: input length must be a power of 2")

    if _ffht_axis0_inplace is not None and arr.dtype in (np.float32, np.float64):
        order = 'F' if arr.ndim == 2 else 'C'
        y = np.array(arr, dtype=arr.dtype, copy=True, order=order)
        _ffht_axis0_inplace(y)
        y /= np.sqrt(float(n))
        return y

    if not _HADAMARD_KERNEL_AVAILABLE:
        raise ImportError("orthonormal_hadamard: neither the FFHT backend nor hadamardKernel is available")

    return np.sqrt(float(n)) * fastwht(arr, N=n, order='hadamard')
